 ...
 *     y[n] = b0 * x[n] + b1 * x[n-1] + b2 * x[n-2] - a1 * y[n-1] - a2 * y[n-2]      
 * </pre>      
 * In this case the feedback coefficients <code>a1</code> and <code>a2</code> must be negated when used with the CortexR4 DSP Library.      
 *      
 * \par      
 ...
 *      
 * \par      
 * Two related functions are provided in the Cortex-R4 DSP library.      
