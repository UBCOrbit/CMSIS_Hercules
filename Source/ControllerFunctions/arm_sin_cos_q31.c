...
{     
  ...
  q31_t ysin0, ysin1;                            /* Nearest output values */     
  q31_t ycos0, ycos1;                            /* Nearest output values */     
  ...
  ysin0 = sinTableQ31[i];     
  ysin1 = sinTableQ31[i + 1u];     
     
  /* thetaminusx0 = theta - x0 */  
  thetaminusx0 = theta - x0;  
  ...
  ycos0 = cosTableQ31[i];     
  ycos1 = cosTableQ31[i + 1u];     
     
  out = (((q63_t) thetaminusx0 * oneByXSpacing) >> 32);  
  ...
  out = (((q31_t) out) << sign_bits);     
    
  /* difference of nearest cosine output value */  
  ycos1 = ycos1 - ycos0;  
  
  acc1 = ((q63_t) ysin1 * out);  
  acc2 = ((q63_t) ycos1 * out);  
