...
{     
  ...
  {     
    ...
    _SIMD32_OFFSET(pSrc16 + (2u * i1)) = (q31_t) ((out2) & 0xFFFF0000) | (out1 & 0x0000FFFF);     
    ...
    _SIMD32_OFFSET(pSrc16 + (2u * i2)) = ((out2) & 0xFFFF0000) | ((out1) & 0x0000FFFF);     
    ...
    _SIMD32_OFFSET(pSrc16 + (2u * i3)) = ((out2) & 0xFFFF0000) | (out1 & 0x0000FFFF);     
  ...
  {     
    ...
    {     
      ...
      {     
        ...
        /* xc' = (xa-xb+xc-xd)* co2 + (ya-yb+yc-yd)* (si2) */     
        /* yc' = (ya-yb+yc-yd)* co2 - (xa-xb+xc-xd)* (si2) */     
        _SIMD32_OFFSET(pSrc16 + (2u * i1)) = ((out2) & 0xFFFF0000) | (out1 & 0x0000FFFF);     
...
{     
  ...
  {     
    ...
    {     
      /*  pSrc[i] <-> pSrc[j]; */     
      /*  pSrc[i+1u] <-> pSrc[j+1u] */     
      in = _SIMD32_OFFSET(pSrc16 +i * 2u);     
      _SIMD32_OFFSET(pSrc16 + i * 2u) = _SIMD32_OFFSET(pSrc16 + j * 2u);     
      _SIMD32_OFFSET(pSrc16 + j * 2u) = in;     
     
      /*  pSrc[i + fftLenBy2p1] <-> pSrc[j + fftLenBy2p1];  */     
      /*  pSrc[i + fftLenBy2p1+1u] <-> pSrc[j + fftLenBy2p1+1u] */     
      in = _SIMD32_OFFSET(pSrc16 + (i + fftLenBy2p1) * 2u);     
      _SIMD32_OFFSET(pSrc16 + (i + fftLenBy2p1) * 2u) = _SIMD32_OFFSET(pSrc16  + (j + fftLenBy2p1) * 2u);     
      _SIMD32_OFFSET(pSrc16 + (j + fftLenBy2p1) * 2u) = in;     
    }     
     
    /*  pSrc[i+1u] <-> pSrc[j+fftLenBy2];         */     
    /*  pSrc[i+2] <-> pSrc[j+fftLenBy2+1u]  */     
    in = _SIMD32_OFFSET(pSrc16 + (i + 1u) * 2u);     
    _SIMD32_OFFSET(pSrc16 + (i + 1u) * 2u) = _SIMD32_OFFSET(pSrc16 + (j + fftLenBy2) * 2u);     
    _SIMD32_OFFSET(pSrc16 + (j + fftLenBy2) * 2u) = in;     
